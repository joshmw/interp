...
def main(args):
    ...
    image_path2 = args.imagepath2 #'{}/{}.jpg'.format(args.inputdir, args.image)
    args.image = image_path.split('/')[-1].split('.')[0]
    args.image2 = image_path2.split('/')[-1].split('.')[0]
    original_image = preprocess_im(image_path)
    original_image2 = preprocess_im(image_path2)
    ...
    #set the interpolation values for each layer
    #interps = np.zeros(len(all_layers)-1)
    #interps = interps + .5
    #interps[-1] = interp
    #interps[-2] = interp
    #interps[-3] = interp
    ...
    texsyn = ts.TextureSynthesis(vgg19, original_image, original_image2, interp, layer_weight, args.nPools, args.layer, args.image, args.image2, tmpDir, args.iterations+1)
    ...
    postprocess_img(tmpDir, args, interp)
...
def postprocess_img(raw, args, interp, steps=None):
    for im in os.listdir(raw):
        if '{}_{}_{}_{}x{}_{}_smp'.format(args.image, interp, args.image2, args.nPools, args.nPools, args.layer) in im  and ('final' in im or 'step_{}.npy'.format(args.iterations) in im or 'step_{}.npy'.format(steps) in im):
            path_to_img = raw+'/'+im
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--imagepath", default="in/grass.jpg")
    parser.add_argument("-j", "--imagepath2", default="in/leaves.jpg")
    parser.add_argument("-t", "--interp", type=float, default=0)
