function [ myscreen ] = interp_mriObj( varargin )
%
% EVENT-RELATED TEXTURES
%  Experiment to map neural responses to various inteprolation images
%
%  Usage: interp_mriObj(varargin)
%  Authors: Josh Wilson
%  Date: 12/29/24
%
...
% add arguments later
getArgs(varargin,{'scan=0', 'testing=0', 'central=1'}, 'verbose=1');
stimulus.scan = scan;
stimulus.debug = testing;
stimulus.central = central;
clear scan testing;
...
if stimulus.scan || true
  myscreen = initScreen('fMRIprojAchkuo2');
else
...
stimulus.interpNames = {{'goldfish', 'lemon'}, {'conch', 'gorilla'}, {'pillow', 'starfish'}, {'pufferfish', 'bagel'}};

stimulus.nInterpFams = length(stimulus.interpNames);
stimulus.imSize = 12;
stimulus.stimXPos = 7;
if central, stimulus.stimXPos = 0; end
stimulus.interpNums = [0 2 4 6 8 10];

%% Select the condition for this run
% Choose which image and which pooling layer to display on this run on each side
stimulus.stimDir = '~/Desktop/interp/goodObjectMorphs/';
...
for i = 1:stimulus.nInterpFams
    ...
    for j = stimulus.interpNums
       imageName = sprintf('%02d.png', j);
       sd = imread(sprintf('%s%s_%s/%s', stimulus.stimDir, imName{1}, imName{2}, imageName));
       %rename image name so you can save in stimulus
       if imageName(end-3:end) == '.png'; imageName = imageName(1:end-4); end
       imageName(imageName=='.') = [];
       imageName = sprintf('%s%s%s', imName{1}, imName{2}, imageName);
       %add texture to stimulus variable
       stimulus.images.synths.(imageName) = genTexFromIm(permute(sd, [2 1 3]), mask);
    end
...
if stimulus.central, task{2}{1}.parameter.interpVal = task{1}{1}.parameter.interpVal - task{1}{1}.parameter.interpVal - 1;; end
...
if task.thistrial.interpVal == -1
  ...
  trialImName = sprintf('%s%s%02d', stimulus.interpNames{task.thistrial.imageClass}{1}, stimulus.interpNames{task.thistrial.imageClass}{2}, task.thistrial.interpVal);
