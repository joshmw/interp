...
%% Load the data
%get args
getArgs(varargin, {'reliability_cutoff=.6', 'r2cutoff=0', 'stdCutoff=5', 'shuffleData=0', 'zscorebetas=1', 'numBoots=250', 'showAvgMDS=50', 'showDistancesSingleTrials=0', 'mldsReps=1', 'plotBig=0', 'doROImlds=0,'...
...
for sub = 1:2:length(roiNames);
    ...
    if size(singleTrials,1) > 2
        ...
        subplot(6,9,ceil(sub/2)), hold on
...
roisToCombine = 1:length(task{1}.roiNames); roisToCombine = roisToCombine(numUsableVoxelsByROI > 2); roisToCombine = roisToCombine(mod(roisToCombine,2)==1);
